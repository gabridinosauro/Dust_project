...
bac = ggplot( geodata, aes(OC_points_percent,nb)) + geom_point() +
        ylab("Niche breadth (B)") + xlab("Occupancy %") +
        theme_classic() + ggtitle("Bact/Arch") + stat_cor(method = "pearson", label.x = 3, label.y = 30, digits = 3)
bac
...
fun = ggplot( geodata, aes(OC_points_percent,nb)) + geom_point() +
  ylab("Niche breadth (B)") + xlab("Occupancy %") +
  theme_classic()+ ggtitle("Fungi") + stat_cor(method = "pearson", label.x = 3, label.y = 30, digits = 3)

a = ggarrange(bac, fun, labels = "AUTO", nrow = 1)

#ggsave("/Users/gabri/OneDrive - University of Arizona/dust_project/Drafts/Review/figreS33.pdf",a , device = "pdf", width= 9, height = 4 )
